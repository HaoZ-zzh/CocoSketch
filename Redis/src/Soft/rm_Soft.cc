...
static int GetKey(RedisModuleCtx *ctx, RedisModuleString *keyName, SKETCH **sketch, int mode)
{
    ...
    if (RedisModule_KeyType(key) == REDISMODULE_KEYTYPE_EMPTY)
    {
        ...
        ERROR("SoftCoco: key does not exist");
    }
...
{
    long long w, d;
    if ((RedisModule_StringToLongLong(argv[2], &d) != REDISMODULE_OK) || d < 1)
    {
        ERROR("SoftCoco: invalid d");
    }
    if ((RedisModule_StringToLongLong(argv[3], &w) != REDISMODULE_OK) || w < 1)
    {
        ERROR("SoftCoco: invalid w");
    }
...
static int Create_Cmd(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    if (RedisModule_KeyType(key) != REDISMODULE_KEYTYPE_EMPTY)
    {
        RedisModule_ReplyWithError(ctx, "SoftCoco: key already exists");
        goto final;
    }
...
static void RdbSave(RedisModuleIO *io, void *obj)
{
    ...
    RedisModule_SaveStringBuffer(io, (const char *)sketch->key_lens, sketch->w * sketch->d * sizeof(size_t));
    for (uint32_t i = 0; i < sketch->w * sketch->d; ++i)
...
static void *RdbLoad(RedisModuleIO *io, int encver)
{
    ...
    assert(tmp == sketch->w * sketch->d * sizeof(uint32_t));

    sketch->key_lens = (size_t *)RedisModule_LoadStringBuffer(io, &tmp);

    sketch->keys = (char **)CALLOC(sketch->d * sketch->w, sizeof(char *));
    for (uint32_t i = 0; i < sketch->w * sketch->d; ++i)
        if (sketch->key_lens[i])
...
int SoftModule_onLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    SKETCHType = RedisModule_CreateDataType(ctx, "Soft-TYPE", SOFT_ENC_VER, &tm);
    if (SKETCHType == NULL)
        return REDISMODULE_ERR;

    RMUtil_RegisterWriteDenyOOMCmd(ctx, "SoftCoco.create", Create_Cmd);
    RMUtil_RegisterWriteDenyOOMCmd(ctx, "SoftCoco.insert", Insert_Cmd);
    RMUtil_RegisterReadCmd(ctx, "SoftCoco.query", Query_Cmd);
    RMUtil_RegisterReadCmd(ctx, "SoftCoco.info", Info_Cmd);
